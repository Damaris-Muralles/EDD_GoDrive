...
// INICIO DE SESION
func Incio_sesion(path string, lista_student_record *doublelist.Lista_doble, cola_student *queue.Cola, pila_record_admin *stack.Pila) {
	...
	if users == "admin" {
		if password == "admin" {
			admin_funcion.Tablero_Admin(path, lista_student_record, cola_student, pila_record_admin)
		} else {
		...
		if !lista_student_record.Esta_vacia() {
			...
			if result {
				...
				if datos_start.Contraseña == password {
					...
					lista_student_record.Modify_pila(carnet, "Se inició sesión", fechayhora)
			...
			fmt.Println("No se ha encontrado usuario")
			time.Sleep(1 * time.Second)
		}
	}
...
`
		...
		switch opcion {
		case 1:
			Incio_sesion(path, &lista_student_record, &cola_student, &pila_record_admin)
			retorno = 0
		default:
			retorno = 1
		}
